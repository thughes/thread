...
namespace boost
{
  ...
  class BOOST_THREAD_SCOPED_LOCKABLE lock_guard
  {
  private:
    Mutex& m;
  ...
  public:
    ...
    explicit lock_guard(Mutex& m_) BOOST_THREAD_EXCLUSIVE_LOCK_FUNCTION(m_) :
      m(m_)
    {
      m.lock();
    }

    lock_guard(Mutex& m_, adopt_lock_t) BOOST_THREAD_EXCLUSIVE_LOCKS_REQUIRED(m_) :
      m(m_)
    {
#if ! defined BOOST_THREAD_PROVIDES_NESTED_LOCKS
...
#endif
    ~lock_guard() BOOST_THREAD_UNLOCK_FUNCTION()
    {
      m.unlock();
    }
  };
